...
if __name__ == "__main__":
    ...
    latent_dim = np.round(np.array(461)*(0.0625,0.125,0.25,0.5,0.75,1.25,1.5,1.75,2)).astype(int)

    #hyperparameter options
    layer = [1,2]
    #selected options from first hyperparameter search
    ...
    for lat in latent_dim:
        ...
        for lay in layer:
            ...
            labeling = "layer_"+str(lay)+"__latent_"+str(lat)+"gross"
    ...
    np.save(file="loss_big/best3",arr=mins)    
    np.save(file="loss_big/best_latent",arr=latbest)   
    ...
    plt.savefig("figures/loss_feature.pdf")
